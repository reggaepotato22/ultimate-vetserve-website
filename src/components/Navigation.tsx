...
const Navigation = () => {
  ...
  return (
    <>
      {/* Top Info Bar — normal flow, scrolls away */}
      ...
      <header className="sticky top-0 z-50">
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-[72px]">
            {/* Logo */}
            <Link to="/" className="flex items-center shrink-0 -ml-1.5">
              <img
                src={logo}
          ...
          {isOpen && (
            <div className="lg:hidden fixed inset-0 top-0 z-50">
              ...
              <div className="relative bg-white border-t border-zinc-100 animate-slide-down mt-[72px] max-h-[calc(100vh-72px)] overflow-y-auto shadow-2xl">
                <div className="flex items-center justify-between px-5 pt-3 pb-1">
                  <p className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Menu</p>
                  <button onClick={() => setIsOpen(false)} className="p-1.5 rounded-lg hover:bg-zinc-100 text-zinc-400 transition-colors" aria-label="Close menu">
                    <X className="h-4 w-4" />
                  </button>
                </div>
                <nav className="flex flex-col gap-0.5 px-2 pb-2">
                  {allMobileLinks.slice(0, 2).map((link) => (
                    <NavLink
                      ...
                      className="flex items-center gap-3 px-4 py-3.5 rounded-xl text-[14px] text-zinc-700 hover:bg-primary/[0.07] hover:text-primary transition-all font-medium"
                      activeClassName="bg-primary/[0.08] text-primary font-semibold"
                    >
                      {link.name}
                    </NavLink>
                  ))}
                  {/* Products with sub-links */}
                  <div className="bg-zinc-50/70 rounded-xl px-3 py-2 mt-1">
                    <NavLink
                      to="/products"
                      onClick={() => setIsOpen(false)}
                      className="flex items-center gap-3 px-3 py-3 rounded-xl text-[14px] text-zinc-700 hover:text-primary transition-all font-medium"
                      ...
                      {productCategories.map((cat) => {
                        ...
                        return (
                          <Link
                            ...
                            to={`/products?category=${cat.slug}`}
                            onClick={() => setIsOpen(false)}
                            className="flex items-center gap-2.5 px-3 py-2.5 rounded-lg text-[12.5px] text-zinc-500 hover:text-primary hover:bg-white transition-all"
                          >
                ...
                <div className="px-4 pt-4 pb-6 mt-3 border-t border-zinc-100 flex flex-col gap-3">
                  <a
                    href="tel:+254724241542"
                    className="flex items-center gap-2 justify-center py-3.5 bg-primary/[0.06] hover:bg-primary/[0.10] border border-primary/15 rounded-xl text-primary font-bold text-sm transition-all"
                  >
